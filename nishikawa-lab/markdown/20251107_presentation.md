...
この互いが切磋琢磨する様子をミニマックスゲームとして捉え定式化する
...
| ケース           | $D$ の予測   | 正解       | 内容                          |
|:----------------|:------------|:----------|:------------------------------|
| 本物($\mathbf{x}$)  | $[D,\, 1-D]$ | $[1,\, 0]$ | 第1項：本物時の負の交差エントロピー |
| 偽物($G(\mathbf{z})$) | $[D,\, 1-D]$ | $[0,\, 1]$ | 第2項：偽物時の負の交差エントロピー |

$D$ は入力 $\mathbf{x}$ が本物である確率を表す

この和は$D$ の2クラス分類の正しさ（交差エントロピーによる識別性能）を表すので、和が大きくなるように $D$ を調整すると $\mathbf{x}$ をより正しく判別できる
...
この時、式(12.28)の最大化は容易だが第2項の勾配が消失してしまう（ $D$ が強すぎると、$D(G(z)) \approx 0$ となり、 $\log(1 - D(G(z)))$ の勾配がほとんど0になる）ので $G$ の学習が進まない
...
この際 $D$ はリプシッツ連続（微分値が発散しないような連続性）であることが要請され、そのために $D$ の各層の重みを常に一定の範囲 $[-c, c]$ に収める処理が採られる
...
$G$ は $\left[ \mathbf{z}^\top , \mathbf{y}^\top \right]^\top$ を入力にとり $\mathbf{x}$ を生成する
...
$D$ は
- 入力： $\mathbf{x}$
- 出力： $\mathbf{x}$ が本物である確率と $\mathbf{x}$ の各クラスのスコア $p(\mathcal{C}_k | \mathbf{x}) (k=1, \dotsb, K)$
...
2値分類の負の損失
$$
\mathcal{L}_s = \mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} [\log D(\mathbf{x})] + \mathbb{E}_{\mathbf{z} \sim p(\mathbf{z})} [\log (1-D(G(\mathbf{z})))] \tag{12.31}
$$

$\mathcal{L}_c$ は $D$ での $\mathbf{x}$ のクラス分類に関する損失で $D$ の出力するクラススコアと正解 $\mathbf{y}$ の交差エントロピー

これで定義される $\mathcal{L}_c + \mathcal{L}_s$ を $G$ に関して小さく、$\mathcal{L}_c - \mathcal{L}_s$ を $D$ に関して小さくする
...
- Initialization: Normal random for latent variable
...
|epoch 1...15|epoch 16...30|
|:-------:|:------------:|
|<img src="../images/GAN_loss_1.png" style="height:23vh;">|<img src="../images/GAN_loss_2.png" style="height:23vh;">|
